...
async def fetch_articles():
    ...
    async with async_playwright() as p:
        ...
        for idx, item in enumerate(latest_2):
            ...
            # 构建详情页 URL
            if info_code:
                if "策略" in (column_type or ""):
                    detail_url = (
                        "https://data.eastmoney.com/report/"
                        f"zw_strategy.jshtml?infocode={info_code}"
                    )
                elif "宏观" in (column_type or ""):
                    detail_url = (
                        "https://data.eastmoney.com/report/"
                        f"zw_macresearch.jshtml?infocode={info_code}"
                    )
                else:
                    detail_url = (
                        "https://data.eastmoney.com/report/"
                        f"zw_stock.jshtml?infocode={info_code}"
                    )
                ...
                try:
                    ...
                    # 等待内容容器出现
                    selectors = "#ContentBody, .newsContent, .txtcont, .report-infocont"
                    try:
                        await page.wait_for_selector(selectors, timeout=20000)
                        print("    正文元素已出现")
                    except Exception:
                        print("    正文元素等待超时，仍尝试提取")
                        ...
                        if encode_url:
                            ...
                            try:
                                ...
                                content_text = await page.evaluate("""
                                    () => {
                                        const sels = [
                                            '#ContentBody', '.newsContent',
                                            '.txtcont', '.report-infocont'
                                        ];
                                        for (const sel of sels) {
                                            const el = document.querySelector(sel);
                                            if (el && el.innerText.trim().length > 50) {
